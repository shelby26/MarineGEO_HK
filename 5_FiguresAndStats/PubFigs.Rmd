...
```{r env_perm, echo=FALSE, warning=FALSE}

library(vegan)
library(dplyr)


env<-read.csv("COI_Final_working/MWQ_monthly.csv",header = T)  ##Import data
env$month<-format(as.Date(env$Dates, format="%Y/%m/%d"),"%m")
env<-env[,c(4,10:22)]
env<-env %>% group_by(Site_ID,month) %>% summarise(mean(Turbidity),mean(Total_Inorganic_Nitrogen),mean(Salinity),mean(Faecal_Coliforms),mean(Dissolved_Oxygen_mgL))
Impact<-c(rep("Low",24),rep("High",36),rep("Med",12),rep("Low",12))
env$Impact<-Impact
colnames(env)<-c("Site_ID","month","Turb","TIN","Sal","FC","DO","Impact")
env<-subset(env,env$Impact != "Med")
env.mat<-env[,3:7]
env.dist<-vegdist(env.mat, method="euclidian")
adonis2(env.dist~Impact,data=env, permutations = 999, strata = env$month)
...
HvL_a_aov<-adonis(HvL_a_dist ~ Impact/Station_ID, data = HvL_a_sam)
HvL_a_aov$aov.tab
